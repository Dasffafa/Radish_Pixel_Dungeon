...
public class HalomethaneFire extends Blob {
            ...
            for (int j = area.top-1; j <= area.bottom; j++) {
                ...
                if (cur[cell] > 0) {
                    ...
                    if (flamable[cell]
                            ...
                            || cur[cell+Dungeon.level.width()] > 0) &&  (Dungeon.hero.pointsInTalent(Talent.FIRE_GLASS) >= 2)) {
                        fire = 4;
                        area.union(i, j);
                    } else {
                        fire = 0;
    ...
    public static void cursed( int pos ) {
        ...
        if(ch != null) {
            if (Dungeon.hero.pointsInTalent(Talent.FIRE_GLASS) > 2) {
                Buff.affect(ch, HalomethaneBurning.class).reignite(ch);
                Buff.detach(ch, HolyLowBurinng.class);
            } else {
                Buff.affect(ch, HolyLowBurinng.class).reignite(ch);
            }
        }
    }
