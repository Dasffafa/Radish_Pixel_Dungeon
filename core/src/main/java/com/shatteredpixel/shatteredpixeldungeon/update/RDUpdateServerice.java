...
public class RDUpdateServerice extends UpdateService {
		...
		UpdateChecker.refreshConfig(new Net.HttpResponseListener() {
			...
			public void handleHttpResponse(Net.HttpResponse httpResponse) {
				if (UpdateChecker.config == null) {
					...
					String latestVersionCode = UpdateChecker.config.get("RAPDVersionCode").asText();
					if (ShatteredPixelDungeon.versionCode < Integer.parseInt(latestVersionCode)) {
						...
						update.versionName = UpdateChecker.config.get("RAPDGameVersion").asText();
