...
public class RDChangesButton extends StyledButton {
	...
	public RDChangesButton(Chrome.Type type, String label) {
		super(type, label);
	...
	public void update() {
		...
		if (!updateShown && Updates.updateAvailable()) {
			...
			text(Messages.get(RDChangesButton.class, "updatefailed"));
		} else if(updateProgressValue<100f && downloadStart){
			text(updateProgress);
		} else if(downloadSuccess) {
			text(Messages.get(RDChangesButton.class, "downloadsuccessyeah"));
		}
	...
	public class WndUpdate extends Window {
		...
		private PlatformSupport.UpdateCallback listener = new PlatformSupport.UpdateCallback() {
			...
			public void onStart(String url) {
				updateProgress = Messages.get(RDChangesButton.class,"downloading");
				downloadStart = true;
			}
			...
			public void onFinish(File file) {
				...
				downloadStart = false;
				RDChangesButton.file = file;
				updateProgress = Messages.get(RDChangesButton.class,"downloadsuccess");
				GLog.p(Messages.get(RDChangesButton.class, "downloadsuccessling"));
			}

			@Override
			public void onError(Exception e) {
				updateProgress = Messages.get(RDChangesButton.class,"downloadingfailed");
		...
		protected void layoutBody(float pos, String message, AvailableUpdateData update) {
			...
			RedButton btn = new RedButton(DeviceCompat.isDesktop() ? Messages.get(RDChangesButton.class,"downloadpc") : Messages.get(RDChangesButton.class,"download1")) {
				@Override
				public void update() {
					if (downloadSuccess) {
						text(updateProgress);
			...
			RedButton btn3 = new RedButton(Messages.get(RDChangesButton.class,"download3")) {
				@Override
				protected void onClick() {
					ShatteredPixelDungeon.switchNoFade(ChangesScene.class);
				}
			...
			RedButton btn4 = new RedButton(Messages.get(RDChangesButton.class,"join")) {
				@Override
				protected void onClick() {
